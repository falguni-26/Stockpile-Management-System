...
class supplierClass:
    def __init__(self,root):
        ...
        txt_search=Entry(self.root,textvariable=self.var_searchtxt,font=("goudy old style",15),bg="lightyellow").place(x=800,y=80)
        btn_search=Button(self.root,text="Search",command=self.search,font=("goudy old style",15),bg="#4caf50",fg="white",cursor="hand2").place(x=950,y=79,width=120,height=30)
    ...
    def add(self):
        ...
        try:
            if self.var_sup_invoice.get()=="":
                ...
                cur.execute("Select * from supplier where inoice=?",(self.var_sup_invoice.get(),))
                row=cur.fetchone()
                if row!=None:
                    messagebox.showerror("Error","Invoice No already assigned,try different ",parent=self.root)
                else:
                    cur.execute("Insert into supplier (inoice,name,contact,desc)values(?,?,?,?)",(
    ...
    def update(self):
        ...
        try:
            if self.var_sup_invoice.get()=="":
                messagebox.showerror("Error","Inoice No Must Be required",parent=self.root)
            else:
                cur.execute("Select * from supplier where inoice=?",(self.var_sup_invoice.get(),))
                row=cur.fetchone()
                if row==None:
                    messagebox.showerror("Error","Invalid Inoice No ",parent=self.root)
                else:
                    cur.execute("Update invoice no set name=?,contact=?,desc=? where inoice =?",(
        ...
        try:
            if self.var_sup_invoice.get()=="":
                ...
                if row==None:
                    ...
                    if op==True:
                        cur.execute("Delete from supplier where inoice=?",(self.var_sup_invoice.get(),))
                        con.commit()
